...
public class ImportBookActivity extends ListActivity {
    ...
    private Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case IMPORT_SUCCEED: {
                    Dialog dialog = new AlertDialog.Builder(ImportBookActivity.this)
                ...
                default: {
                    DataAccess data = new DataAccess(ImportBookActivity.this);
                    DataAccess.bookID = "";
                    DataAccess.bookID = result;
                    if (!DataAccess.bookID.equals(""))
                        data.DeleteBook();
                    DataAccess.bookID = "";
                    Toast.makeText(ImportBookActivity.this, "导入词库失败", Toast.LENGTH_LONG)
                            .show();
                    break;
                }
        ...
        confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (newname.getText().toString().equals("")) {
                    dialog.cancel();
                    Toast.makeText(ImportBookActivity.this, "词库名称不能为空", Toast.LENGTH_LONG)
                            .show();
                }
                try {
                    ...
                    dialog.cancel();
                    Toast.makeText(ImportBookActivity.this, "list容量格式不正确（请输入大于0的整数）", Toast.LENGTH_LONG)
                            .show();
                }
                dialog.cancel();
                ...
                final Thread thread = new Thread(new Runnable() {
                    public void run() {
                        ...
                        try {
                            ...
                            result = "";
                            if (OOB.ImportBook(ImportBookActivity.this, fileNames[position], newname.getText().toString(), Integer.parseInt(numOfEachList.getText().toString()), order)) {
                                Message m = new Message();
                                m.what = ImportBookActivity.IMPORT_SUCCEED;
                                ImportBookActivity.this.mHandler.sendMessage(m);
                            } else {
                                Message m = new Message();
                                m.what = ImportBookActivity.IMPORT_FAIL;
                                ImportBookActivity.this.mHandler.sendMessage(m);
                            }
                ...
                thread.start();
                myDialog = new ProgressDialog(ImportBookActivity.this);
